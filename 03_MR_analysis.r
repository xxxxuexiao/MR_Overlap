...
idx = temp[5]# ith column for betahat_y of Y0, Y25, Y50, Y75 or Y100 for following MR analysis between X (idx=2,3,4,5)
decor = decor_vec[temp[6]]
winner = winner_vec[temp[7]]

nohup Rscript 03_MR_analysis.r 
...
mynames = c("X", "Y0", "Y25", "Y50", "Y75", "Y100")
...
for (repind in 1:100){
	...
	if(numIV>2){
		if(decor==FALSE && winner==FALSE){		
			...
			betahat_x.flt = as.vector(betahat_100sim[[repind]][[idx-1]][,1]) # idx-1=1,2,3,4 for X_Y0, X_Y25, X_Y50, X_Y75
		...
		tryCatch({
			...
			if (nx<=2e5){
				...
				if (class(res)!="try-error"){
					...
					print(paste("Rep",repind,"numIV",numIV, "with try-error"))					
				}
				print(res)
				rm(res)
			}
